...
def getMinimalSet(sortedSuite):
	...
	mat = [[int(lineCoverage[j][i]["@hits"]) for i in xrange(lines)] for j in xrange(cases)]
	lineNumbers = [[int(lineCoverage[j][i]["@number"]) for i in xrange(lines)] for j in xrange(cases)]
	linesCovered = mat[0]
	for i in xrange(1,cases):
		newTest = [mat[i][j]+linesCovered[j] for j in xrange(lines)]
		if linesCovered.count(0) > newTest.count(0):
			linesCovered = newTest
			minSet.append(keys[i])

	for i in xrange(cases-1,-1,-1):
		if keys[i] in minSet:
			newTest = [linesCovered[j]-mat[i][j] for j in xrange(lines)]
			if linesCovered.count(0) == newTest.count(0):
				linesCovered = newTest
